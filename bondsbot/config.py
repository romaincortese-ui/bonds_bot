...
class BondsConfig:
    ...
    def from_env(cls) -> "BondsConfig":
        ...
        return cls(
            ...
            max_portfolio_dv01_nav_10bp=_float("MAX_PORTFOLIO_DV01_NAV_10BP", 0.0065),
            max_country_dv01_nav_10bp=_float("MAX_COUNTRY_DV01_NAV_10BP", 0.0039),
            max_tenor_dv01_nav_10bp=_float("MAX_TENOR_DV01_NAV_10BP", 0.0026),
            ...
            prediction_overlay_enabled=_bool("BONDS_PREDICTION_OVERLAY_ENABLED", False),
            prediction_overlay_state_file=Path(
                os.environ.get(
                    "BONDS_BACKTEST_PREDICTION_STATE_FILE",
                    os.environ.get("BONDS_PREDICTION_STATE_FILE", ""),
                )
            ),
            prediction_overlay_stale_seconds=max(1, _int("BONDS_PREDICTION_STALE_SECONDS", 60)),
            prediction_overlay_fallback_mode=os.environ.get("BONDS_PREDICTION_FALLBACK_MODE", "neutral").strip().lower(),
            prediction_overlay_min_favourable_probability=max(0.0, min(1.0, _float("BONDS_PREDICTION_MIN_FAVOURABLE_PROBABILITY", 0.50))),
            prediction_overlay_min_posterior=max(0.0, min(1.0, _float("BONDS_PREDICTION_MIN_POSTERIOR", 0.50))),
            prediction_overlay_event_given_success=max(0.0, min(1.0, _float("BONDS_PREDICTION_EVENT_GIVEN_SUCCESS", 0.60))),
            prediction_overlay_kelly_base_fraction=max(0.001, _float("BONDS_PREDICTION_KELLY_BASE_FRACTION", 0.04)),
            prediction_overlay_max_size_multiplier=max(0.0, _float("BONDS_PREDICTION_MAX_SIZE_MULTIPLIER", 1.0)),
            prediction_overlay_score_scale=max(0.0, _float("BONDS_PREDICTION_SCORE_SCALE", 20.0)),
